...
/**
 * Canonical DDL builder -- the single source of truth for "what the production
 * database actually looks like."
 *
 * Production schema is applied in two layers:
 *   1. The committed Drizzle migrations (0000-0003) create the base tables
 *      (users / places / tours / experiences / payments / matches / messages).
 *   2. The hand-written `scripts/create-*.ts` + `scripts/add-*.ts` path adds
 *      everything since (accounts, marketplace, products, chat, fixed tours,
 *      crossover, coupons, sessions, ...). Those scripts use raw idempotent SQL
 *      (not `drizzle-kit push`, which is interactive/TTY-hostile).
 *
 * `SCHEMA_DDL` below mirrors that second layer as one ordered, idempotent list.
 * It is consumed by:
 *   - this script's `applyAllDdl`, which builds a full DB from scratch against a
 *     real Postgres (the CI `schema-drift` job's DB-A, and `pnpm db:check`); and
 *   - `src/test/setup.ts`, which replays it into PGlite for the unit/integration
 *     suite.
 *
 * Keeping both consumers on this one list removes the previously hand-mirrored
 * DDL block in `setup.ts` (the M15 drift surface): there is now exactly one
 * place to edit when the create-* / add-* scripts change.
 *
 * NOTE ON FIDELITY: this list is a faithful transcription of the
 * create-* / add-* scripts, not a runtime import of them (those scripts each open their own
 * `postgres` connection and `process.exit` on load, and cannot target PGlite).
 * The `schema-drift` CI job is what guards this transcription against the
 * Drizzle schema (`schema.ts`) drifting away from it.
 */
export const SCHEMA_DDL: string[] = [
  // `accounts` is created by scripts/create-accounts-table.ts.
  ...
  // Customized Tour Template catalog (see scripts/create-customized-tour-templates.ts).
  // Themed day-plan inspiration matrix matched the same way as fixed tours.
  ...
  `CREATE INDEX IF NOT EXISTS idx_customized_tour_templates_theme ON customized_tour_templates(theme)`,
  `CREATE INDEX IF NOT EXISTS idx_customized_tour_templates_active ON customized_tour_templates(is_active)`,
...
/**
 * Build the full canonical schema in `connectionString`: committed Drizzle
 * migrations first, then the create-* / add-* DDL layer. Idempotent.
 */
export async function applyAllDdl(connectionString: string): Promise<void> {
  ...
  try {
    await migrate(dbi, { migrationsFolder: MIGRATIONS_FOLDER });
    for (const stmt of SCHEMA_DDL) {
      await dbi.execute(sql.raw(stmt));
    }
  } finally {
    await client.end();
  }
...
// Run directly: `tsx scripts/apply-all-ddl.ts` (uses DATABASE_URL).
...
if (invokedDirectly) {
  ...
  applyAllDdl(dbUrl)
    .then(() => {
      console.log("apply-all-ddl: schema built.");
      process.exit(0);
    })
    .catch((err) => {
      console.error(err);
