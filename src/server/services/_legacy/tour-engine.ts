...
import { computeTourPrice, TOUR_PRICING } from "@/lib/pricing";
...
export async function generateTour(request: TourRequest, userProfile: Record<string, unknown>) {
  ...
  const priceBySize = computeTourPrice({
    withHost: request.withHost,
  ...
  return {
    ...
    estimatedCost: { min: priceBySize * 0.8, max: priceBySize * 1.2, currency: TOUR_PRICING.currency },
